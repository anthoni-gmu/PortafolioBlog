...
class EditBodyPostForm(forms.ModelForm):
    ...
    body = forms.CharField(
        widget=forms.Textarea(attrs={
            'class': 'w-full bg-white bg-opacity-50 rounded border border-gray-300  focus:bg-white focus:ring-2  h-32 text-base outline-none text-gray-700 py-1 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out',
            'placeholder': 'Edite su comentario'
        }),
        required=True
    )

    imagen = forms.ImageField(label='Portada', required=False, widget=forms.FileInput(attrs={
        'class': 'w-full px-3 py-2 text-sm leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline '
    }))
    class Meta:
        model = BodyPost
